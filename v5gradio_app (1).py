...
async def _analyze(file_obj, pasted_text):
    ...
    yield gr.update(visible=True), gr.update(), gr.update(), gr.update(), gr.update(), gr.update()
    ...
    summary = _summary_markdown(summary_stats)
    ...
    yield gr.update(visible=False), summary, chart, csv_path, df, cache_notice
...
def build_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="ITSM Quality Analysis Agent") as demo:
        ...
        analyze_btn.click(
            ...
            outputs=[agent_progress, summary_md, category_chart, download_file, full_results_state, cache_notice],
        ).then(
            fn=_refresh_view,
